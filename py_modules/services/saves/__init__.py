...
The package's public API is the ``SaveService`` façade — composes the
save-sync sub-services (state, sync_engine, status, versions, slots)
and exposes the callable surface consumed by the Decky entrypoints.
RomM communication goes through ``RommApiProtocol``; no ``import decky``
