...
| per-ROM saves, playtime                                                                                          | save_sync_state.json                         | 3      | DB (`rom_save_sync_states`/`rom_save_files`, `rom_playtime`) | Per-ROM state with invariants. Unchanged.                                                                                                      |
...
**The Repository Protocol set (#782) shrinks 12 → 9:** `Rom`, `RomInstall`, `RomMetadata`, `Playtime`,
`RomSaveSyncState`, `BiosFile`, `FirmwareCache`, `SyncRun`, `KvConfig`. (Dropped: `SyncSettings`, `Platform`, `Device`.)
