...
`retrodeck.json`, `retroarch.cfg`, a bundled `core_defaults.json`, and more. Each source has its own format, its own
update cycle, and its own authoritative domain. None of them is redundant with another, even when they describe
overlapping concepts like "core name" or "supported extensions". (ES-DE's `gamelist.xml` was one such source until #947
dropped it — the plugin no longer reads or writes it; see the [gamelist.xml row](#current-parsers) below.)
...
- **ES-DE** keeps its system definitions in `es_systems.xml` and display-oriented labels it chose to include — the
  plugin reads its **default** core for a system from here. (ES-DE also keeps per-game and per-system core choices in
  `gamelist.xml`, but the plugin no longer reads or writes that file — core deviations are the plugin's own state; see
  [Core and Emulator Selection](core-emulator-selection.md).)
...
| --------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| What's the **default** core for **system X**?                               | ES-DE `es_systems.xml` + bundled `core_defaults.json`                                                            | ES-DE declares the default; `core_defaults.json` is the static fallback when the live file can't be read. The plugin reads it but never the gamelist.    |
...
| Which core will **ROM Y actually launch with** (active core)?               | `ActiveCoreResolver`: per-game DB → per-platform `settings.json` → es_systems.xml default → `core_defaults.json` | One resolver folds the plugin's two deviations over the ES-DE default; the launched core is baked from the same answer.                                  |
...
| `gamelist.xml`                 | XML                     | _none — dropped in #947_                                            | ⛔ no longer read or written                      | **Removed.** The plugin used to read the system-level `<alternativeEmulator>` and write it from the System page; it now neither reads nor writes the gamelist. Core deviations are the plugin's own state — per-platform in `settings.json` `platform_cores`, per-game in the DB (see [Core and Emulator Selection](core-emulator-selection.md)). |
...
The ES-DE `es_systems.xml` parser (`CoreResolver`) keeps its XML parsing inline rather than in a separate pure-domain
