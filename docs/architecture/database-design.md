...
With the aggregate set, the schema, the Repository Protocols, and the runtime UoW in place, the service cutover ([#784](https://github.com/danielcopper/decky-romm-sync/issues/784)) lands vertical by vertical. Already migrated: firmware, downloads + the launcher read path, the saves vertical, the library/roms slice (registry → `roms`, sync lifecycle → `sync_runs`, the platform-name cache → `kv_config`), the metadata slice (`rom_metadata` written by the reporter's per-unit commit; `MetadataService`/`GameDetailService` read it from SQLite), the playtime slice (`PlaytimeService` records sessions and reconciles RomM-note totals through `rom_playtime`), and the rom-removal + startup-healing slice (`RomRemovalService` and `StartupHealingService.prune_stale_installed_roms` read/delete `rom_installs` through the UoW; the JSON `installed_roms` dict is no longer read by either). Still downstream:

- **Teardown** — once the last consumer of each JSON-era persister/state class has moved over (`SaveSyncState.playtime` now has no live reader; `SaveSyncState.saves` is still read by `GameDetailService`, and `installed_roms` is still read by `migration.py` and `game_detail.py`), the dead persisters, the `RegistryStoreAdapter`/`MetadataCacheStoreAdapter` JSON stores, `domain/save_state.py`, and the in-memory state dict are deleted.
