...
- **playtime** — `PlaytimeService` records sessions and reconciles the cross-device total through the `rom_playtime`
  ...
  offline-safe). Opening a game's detail page flushes the outbox, then folds the summed server-session total into
  `rom_playtime` via `reconcile_total` — total-only, monotonic `max()` (ADR-0018).
...
| `Playtime` (`domain/playtime.py`)                 | `total_seconds`, `session_count`, `last_session_start?`, `last_session_duration_sec?`, `pending_sessions{}` (a `PendingPlaySession` outbox row per session-start timestamp)               | Per-ROM, owned by PlaytimeService. Spans two tables — the scalars (`rom_playtime`) plus the pending-session outbox (`rom_playtime_sessions`). Independent lifecycle from saves (`session_lifecycle.py` already treats them as separate concerns). Keyed by `rom_id`. `total_seconds` folds in via `reconcile_total` (monotonic `max()`, never regresses): session-end folds the local duration AND enqueues the session into the outbox; reconcile-on-view flushes the outbox to RomM's native `/api/play-sessions` ingest and folds the summed cross-device server total back in. The outbox dequeues on a successful (or duplicate) ingest and survives offline; each row is grouped by its stored `device_id` for the POST, and its `attempts` counter drives a bounded-retry quarantine (dropped after the threshold of consecutive `error` verdicts) so a permanently-rejected session cannot wedge the outbox (ADR-0018).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
- the **playtime** slice (`PlaytimeService` records sessions into `rom_playtime` + the `rom_playtime_sessions` outbox
  and reconciles the cross-device total through RomM's native `/api/play-sessions` ingest — session-end
  fold+enqueue+flush, pull-only reconcile-on-view, ADR-0018).
