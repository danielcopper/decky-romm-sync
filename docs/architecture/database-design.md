...
With the aggregate set, the schema, the Repository Protocols, and the runtime UoW in place, the service cutover ([#784](https://github.com/danielcopper/decky-romm-sync/issues/784)) lands vertical by vertical. Already migrated: firmware, downloads + the launcher read path, the saves vertical, and the library/roms slice (registry → `roms`, sync lifecycle → `sync_runs`, the platform-name cache → `kv_config`). Still downstream:

- **The remaining per-ROM verticals** — metadata (`rom_metadata`) and playtime (`rom_playtime`) still read the legacy JSON-era state classes until they cut over.
