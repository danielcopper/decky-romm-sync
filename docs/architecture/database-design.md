...
With the aggregate set, the schema, the Repository Protocols, and the runtime UoW in place, the service cutover ([#784](https://github.com/danielcopper/decky-romm-sync/issues/784)) lands vertical by vertical. Already migrated: firmware, downloads + the launcher read path, the saves vertical, the library/roms slice (registry → `roms`, sync lifecycle → `sync_runs`, the platform-name cache → `kv_config`), and the metadata slice (`rom_metadata` written by the reporter's per-unit commit; `MetadataService`/`GameDetailService` read it from SQLite). Still downstream:

- **The remaining per-ROM vertical** — playtime (`rom_playtime`) still reads the legacy JSON-era state class until it cuts over.
- **Teardown** — once the last consumer of each JSON-era persister/state class has moved over, the dead persisters, the `RegistryStoreAdapter`/`MetadataCacheStoreAdapter` JSON stores, `domain/save_state.py`, and the in-memory state dict are deleted.
