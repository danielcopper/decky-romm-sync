...
| `es_de_paths.py`                                                                  | ES-DE gamelist `<path>` identity for a ROM — `gamelist_entry_path` (derives the system-relative path from `file_path` + `rom_dir`, basename for single-file, dir-relative for folder-backed) + `normalize_gamelist_path` (prefix-strip for comparison) |
