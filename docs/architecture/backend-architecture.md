...
- **No fallback**: if the library cannot load, `GavelNativeLoadError` propagates so `bootstrap()` aborts and the plugin
  ...
  `domain.sync_action` kernels are **not** a runtime fallback; they are retained as the differential oracle and the
  gavel-vector target in tests (`tests/adapters/test_gavel_native.py`,
  `tests/domain/test_sync_action_gavel_table_vectors.py`), so the shipped binary is proven to match the same contract
  the Python kernels are held to. The differential crosses local-file forms (missing, unmeasurable, 0-byte, shrunk, both
  mtime directions) against all three `device_syncs` branches, both timestamp-parseability outcomes, and every baseline
  combination. It has exactly one documented exclusion: `local_file={}` on an empty slot, where the core uploads (a
  present-but-unmeasurable file) and the in-tree kernel's truthiness test reads no file at all. The core's reading is
  the correct one and `MatrixExecutor._build_local_input` never produces that shape.
...
| Module                                                                            | Role                                                                                                                                                                                                                                                                                                                               |
| --------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `sync_action.py`                                                                  | The `SyncAction` union (`Skip` / `Upload` / `Download` / `Conflict`) the whole vertical dispatches on, plus `compute_sync_action` / `resolve_upload_conflict` — the two decision kernels, now the compiled core's differential oracle rather than runtime code. See [Save File Sync Architecture](save-file-sync-architecture.md). |
...
| **GameDetailService**       | `BiosChecker`, `AchievementsReader` (cross-service), `Clock`, `UnitOfWorkFactory` (one read UoW over `roms` / `rom_installs` / `rom_save_sync_states` / `rom_metadata` / `kv_config`)                                                                                                                                                                                                                                                                                                                        |
