...
| `game_process.py`                                                          | `GameProcessControl` — resolves a flatpak app's host PIDs via the per-user instance registry (`info` / `bwrapinfo.json`) plus the `/proc` child walk, and signals them. Direct reads + `os.kill`, no subprocess; fail-soft on every read                                               |
...
rather than in-tree Python. `GavelNativeAdapter` loads `py_modules/native/libgavel-x86_64-linux.so` via `ctypes` at
construction and is itself the `ResolveUploadConflictFn` seam (`services/protocols/infra.py`) injected — through
`SaveServiceConfig` → `SyncEngineConfig` → `MatrixExecutor` — into the sole call site, the upload-409 backstop in
`services/saves/sync_engine/matrix.py`.

- **What ships**: `py_modules/native/libgavel-x86_64-linux.so` (romm-gavel `v0.3.0`, a freestanding build with zero
...
| **GameProcessService**      | `GameProcessControl` (process discovery + signals), `Sleeper` (the grace window — the service owns no clock), and RetroDECK's flatpak app id from the single `domain.shortcut_data.RETRODECK_APP_ID` constant the launch command is built from                                                                                                                                                                                                                                                               |
