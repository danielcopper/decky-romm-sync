...
| `playtime.py`             | PlaytimeService — session recording into `rom_playtime` + the `rom_playtime_sessions` outbox, native play-session ingest (session-end fold+enqueue+flush to `/api/play-sessions` grouped per stored `device_id` with a bounded-retry quarantine + pull-only reconcile-on-view, ADR-0018); owns the durable `kv_config["playtime_scope_notice"]` re-sign-in flag (a reconcile 403 sets it, a later 200 or a fresh sign-in clears it) surfaced via `get_playtime_scope_notice`                                                                             |
