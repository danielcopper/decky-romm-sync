...
| **RomAdoptionService**      | `RommRomReader`, `DownloadFileStore` (the `stat`, the top-level listing, the directory scan, the hashing), `AdoptionMoveStore` (the rename across the roms/saves/states trees), `RetroDeckPaths`, `SystemResolver`, `SystemM3uSupportFn`, `SystemSupportedExtensionsFn`, `RetroArchSaveLayoutProvider` + `RetroArchSavestateLayoutProvider` (read separately — RetroArch sorts them independently), `ActiveCoreReader` + `CoreNameProviderFn` (the per-core save subdirectory, only when a layout says there is one), `RomInstallRecorder` peer, `EventEmitter` + `Clock` (throttled `verify_progress` frames) |
