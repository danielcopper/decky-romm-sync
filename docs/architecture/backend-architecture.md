...
| `cores.py`                | CoreService — available-core lookup, per-game core pin/clear (`roms.emulator_override`), per-system core write (gamelist `<alternativeEmulator>`); see [Core and Emulator Selection](core-emulator-selection.md)                                                          |
| `active_core_resolver.py` | ActiveCoreResolver — the single per-ROM read seam: `active_core_for_rom(rom_id)` folds the DB override over the system layer (override → system `<alternativeEmulator>` → es_systems default → core_defaults). Every per-game core read + every launch bake draws from it |
