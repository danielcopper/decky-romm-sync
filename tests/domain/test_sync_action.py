...
def test_is_current_true_local_diverged_returns_conflict():
    """Argosy deviation: detect offline edits eagerly even when is_current=true."""
    server = _server_save(device_syncs=[_device_sync(DEVICE_ID, is_current=True)])
    result = compute_sync_action(
        local_file=_local(),
        server_saves_in_slot=[server],
...
def test_no_device_entry_local_newer_returns_skip():
    ...
    assert result == Skip(reason="local_newer_no_entry")
...
def test_deferred_state_unchanged_returns_skip():
    server = _server_save(
        save_id=42,
    ...
    files_state = {
        ...
        "deferred": {
            "server_save_id": 42,
            "server_updated_at": "2024-06-01T10:00:00+00:00",
            "deferred_at": "2024-06-02T08:00:00+00:00",
        },
    }
    result = compute_sync_action(
        local_file=_local(),
        ...
        device_id=DEVICE_ID,
        local_hash="DIFFERENT",  # would normally trigger Conflict
    )
    assert result == Skip(reason="deferred_unchanged")


def test_deferred_state_server_changed_re_fires_conflict():
    """Stale defer metadata (different updated_at) → falls through to normal logic."""
    server = _server_save(
        save_id=42,
        updated_at="2024-06-05T10:00:00+00:00",  # newer than deferred snapshot
        device_syncs=[_device_sync(DEVICE_ID, is_current=False)],
    )
    files_state = {
        "last_sync_hash": "abc",
        "deferred": {
            "server_save_id": 42,
            "server_updated_at": "2024-06-01T10:00:00+00:00",  # stale
            "deferred_at": "2024-06-02T08:00:00+00:00",
        },
    }
    result = compute_sync_action(
...
def test_last_sync_hash_none_skips_divergence_check():
    ...
    assert result_current == Skip(reason="synced")
...
def test_no_device_entry_local_mtime_equals_server_epoch_returns_skip():
    """Boundary: `local_mtime == server_epoch` must satisfy the `>=` semantics
    and result in Skip(local_newer_no_entry), not Download.
    """
